    ...
    def generate(
        ...
        **kwargs
        ...
        while gen_steps < allowed_gen_steps:
            ...
            # print("DEBUG")
            # print("base_next_token_logits", base_next_token_logits.shape)
            # print("expert_next_token_logits", expert_next_token_logits.shape)
            # print("antiexpert_next_token_logits", antiexpert_next_token_logits.shape)
            ...
            modified_logits = base_next_token_logits.clone()

            # Only update the first vocab_size logits.
            if extra_prompt_appended:
                warmup_alpha = 0.0
            else:
                warmup_alpha = (gen_steps / 100) * self.alpha if gen_steps < 100 else self.alpha
            modified_logits[:, :vocab_size] = (
                base_next_token_logits[:, :vocab_size] +
                warmup_alpha * (expert_next_token_logits[:, :vocab_size] - antiexpert_next_token_logits[:, :vocab_size])
            )

            next_token_logits = modified_logits
